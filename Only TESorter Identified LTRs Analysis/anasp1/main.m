...
ltrname = importltrname('ltrsequences_anasp1.txt');

%import LTR data from LTRHarvest
ltrlengths = importltrlength('ltrdata_anasp1.txt');

%Import BLAST transcriptome matches
BLASTresults = importdatablast('blastresultsanasp1.txt');
rpkmresults = importrpkm('blastresultsanasp1.txt');

%Import clustered LTRs
cdhit = importdatacdhit('cdhitresultsaSaL70_anasp1.txt');

%Import LTR classification from TEsorter
ltrclassification = importtesorter('tesorterresults_anasp1.txt');
...
writetable(clusteredltrs_ltrdata,'ltrdata_anasp1.xlsx')

saveas(graph_transcribedltrs, 'transcribedclusters_anasp1.png')
saveas(graph_all_ltrs, 'all_ltrs_clustergraph_anasp1.png')
saveas(graph_rpkm_vs_cluster, 'rpkmclustergraph_anasp1.png')
saveas(gcf, 'piechart_anasp1.png')

saveas(graph_transcribedltrs, 'transcribedclusters_anasp1.svg')
saveas(graph_all_ltrs, 'all_ltrs_clustergraph_anasp1.svg')
saveas(graph_rpkm_vs_cluster, 'rpkmclustergraph_anasp1.svg')
saveas(gcf, 'piechart_anasp1.svg')

%% function to find percent classified LTRs from TESorter
classvsunclass = classvsunclasspie(ltrclassification, ltrname);

saveas(gcf, 'classvsunclass_anasp1.svg')
saveas(gcf, 'classvsunclass_anasp1.png')